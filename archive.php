...
$show_breadcrumb = apply_filters('cozycorner_show_breadcrumb_on_archive_post', true);
$show_page_title = apply_filters('cozycorner_show_page_title_on_archive_post', true);
$page_title = '';
...
if( $show_page_title ){
	switch( true ){
		case is_day():
			$page_title = esc_html__( 'Day: ', 'cozycorner' ) . get_the_date();
		break;
		case is_month():
			$page_title = esc_html__( 'Month: ', 'cozycorner' ) . get_the_date( esc_html_x( 'F Y', 'monthly archives date format', 'cozycorner' ) );
		break;
		case is_year():
			$page_title = esc_html__( 'Year: ', 'cozycorner' ) . get_the_date( esc_html_x( 'Y', 'yearly archives date format', 'cozycorner' ) );
		break;
		case is_search():
			$page_title = esc_html__( 'Search Results for: ', 'cozycorner' ) . get_search_query();
		break;
		case is_tag():
			$page_title = esc_html__( 'Tag: ', 'cozycorner' ) . single_tag_title( '', false );
		break;
		case is_category():
			$page_title = esc_html__( 'Category: ', 'cozycorner' ) . single_cat_title( '', false );
		break;
		case is_404():
			$page_title = esc_html__( 'OOPS! FILE NOT FOUND', 'cozycorner' );
		break;
		default:
			$page_title = esc_html__( 'Archives', 'cozycorner' );
		break;
	}
}

cozycorner_breadcrumbs_title($show_breadcrumb, $show_page_title, $page_title);
?>
